...
- 메시지에 답장한 뒤 `/msgidd`를 입력하면 답장 대상 메시지의 고유 ID를 알려줍니다.
- 봇이 들어가 있는 개인 대화, 그룹, 슈퍼그룹 또는 채널에서 `/chatidd`를 입력하면
  현재 채팅의 숫자 ID를 알려줍니다. 채널에서는 봇을 관리자로 추가하고 메시지 전송 권한을
  부여해야 합니다.
...
`/제휴` 명령을 입력하면 유흥 및 업자 제휴 리스트로 이동하는 링크를 안내합니다.
...
- `TELEGRAM_PARTNERS_MESSAGE_ID`: 주기적으로 수정할 제휴업체 목록 메시지 ID. 비워 두면
  자동 수정 기능을 사용하지 않습니다.
- `TELEGRAM_PARTNERS_PHOTO_PATH`: 제휴업체 목록 사진 경로(선택 사항,
  기본값 `assets/group-welcome.png`)
...
사진 메시지를 하나 게시합니다. Telegram은 기존 텍스트 메시지를 사진 메시지로 바꿀 수
없습니다. 사진 메시지에 답장하여 `/msgidd`로 확인한 숫자를
`TELEGRAM_PARTNERS_MESSAGE_ID`에 설정하고 봇을 재시작하면 됩니다. 봇은 시작 직후와 이후
설정된 간격마다 활성 제휴업체를 DB에서 다시 조회하여 **같은 메시지**의 사진과 설명을
...
순서는 바뀌지 않습니다. 광고 활성 종료 시각은 DB에 저장된 UTC 기준으로 비교합니다.
...
봇 자체가 그룹, 슈퍼그룹 또는 채널에 새로 추가되면 콘솔에 `[chat-entry]` 로그가
출력됩니다. 로그에는 채팅 ID, 유형, 제목, 공개 사용자명, 설명, 초대 링크, 봇의 권한
상태와 봇을 추가한 사용자의 정보가 포함됩니다. 기존 채팅에서 단순히 관리자 권한만
변경된 경우에는 중복해서 출력하지 않습니다.

봇이 추가된 채팅 정보와 개인 채팅에서 `/start`를 사용한 사용자 정보는 기본적으로
`data/bot-usage.json`에 함께 저장됩니다. 채팅 ID와 사용자 고유번호를 키로 사용하므로
같은 채팅 또는 사용자가 다시 기록될 때 항목이 중복 생성되지 않고 최신 정보와 마지막
이용 시각만 갱신됩니다. 최초 추가·이용 시각은 계속 보존됩니다. 이 파일에는 사용자
정보가 포함되므로 외부에 공개하지 말고 파일 접근 권한을 안전하게 관리해야 합니다. JSON
문서 전체는 파일에 한 줄로 기록됩니다.
...
- 개인 대화에서 `/start`를 받으면 해당 `/start` 명령만 남기고 그보다 앞선 메시지를
  100개 단위로 모두 정리합니다. Telegram의 삭제 가능 기간 등 API 제한에 해당하는
  메시지는 제외됩니다.
